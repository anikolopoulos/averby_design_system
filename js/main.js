...
function initializeTheme() {
  ...
  function updateThemeToggleIcons() {
    const toggles = document.querySelectorAll(
      '[data-testid="theme-toggle"], #toggleDark, [data-testid="mobile-theme-toggle"], #mobileTopBarThemeToggle'
    );
  ...
  railNav.innerHTML = AppConfig.NAVIGATION.map((item) => {
    ...
    return `
      <button
        ...
        class="flex flex-col items-center justify-center text-center rounded-xl min-h-[48px] min-w-[48px] transition-all duration-[180ms] ease-out ${isActive} group gap-1 px-2 py-3 text-[13px] font-semibold text-neutral-700 dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-600 hover:scale-[1.02] focus:outline-none active:scale-[0.96] ${
      isActive.includes("active")
        ? "bg-primary-50 dark:bg-primary-900/40 text-primary-600 dark:text-primary-300 shadow-sm dark:shadow-primary-900/30"
        : ""
    }"
        aria-label="${item.label}"
      >
        <span class="w-6 h-6 mb-1 material-symbols-rounded text-2xl transition-all duration-200 group-[.active]:text-primary-600 dark:group-[.active]:text-primary-400 group-[.active]:icon-fill-1 group-hover:icon-weight-600 group-hover:icon-size-28 ${iconClass}" translate="no">${
      item.icon
    }</span>
        <span class="text-[13px] font-bold transition-all duration-200 group-[.active]:text-primary-600 dark:group-[.active]:text-primary-400">${
          item.label
        }</span>
      </button>
    `;
  ...
  mobileNav.innerHTML = AppConfig.NAVIGATION.map((item) => {
    ...
    return `
      <button
        ...
        class="flex flex-col items-center justify-center text-center rounded-xl min-w-[48px] min-h-[48px] transition-all duration-[180ms] ease-out ${isActive} group flex-1 text-xs font-medium p-1 text-neutral-600 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:bg-neutral-600/50 hover:scale-105 focus:bg-neutral-100/80 dark:focus:bg-neutral-700/60 focus:outline-none focus:ring-2 focus:ring-primary-500/50 dark:focus:ring-primary-400/60 focus:ring-offset-2 focus:ring-offset-transparent dark:focus:ring-offset-neutral-900 ${
      isActive.includes("active")
        ? "text-primary-600 dark:text-primary-300 bg-primary-50/80 dark:bg-primary-900/40 shadow-sm dark:shadow-primary-900/30 border border-transparent"
        : ""
    }"
        role="tab"
        ...
        aria-label="${label}"
      >
        <span class="material-symbols-rounded text-2xl transition-all duration-200 group-[.active]:icon-fill-1 group-hover:icon-weight-600 group-hover:icon-size-28 ${iconClass}" translate="no">${
      item.icon
    }</span>
...
function setupCopyButtons() {
  const codeBlocks = document.querySelectorAll(
    "code.block, code:not(.caption-text)"
  );
...
function addCopyButtonToCode(codeBlock) {
  ...
  button.className =
    "absolute top-2 right-2 p-2 rounded-lg bg-white dark:bg-neutral-800 shadow-sm hover:shadow-lg transition-shadow border border-neutral-200 dark:border-neutral-700";
  button.innerHTML =
    '<span class="material-symbols-rounded text-sm text-neutral-600 dark:text-neutral-400">content_copy</span>';
  button.onclick = (e) => {
    e.preventDefault();
...
function openInset() {
  if (inset) inset.dataset.open = "true";
}
function closeInset() {
  if (inset) inset.dataset.open = "false";
}
...
function makeToast({
  ...
  const glassMap = {
    success:
      "bg-emerald-500/20 text-neutral-900 dark:bg-emerald-400/10 dark:text-neutral-50 ring-1 ring-emerald-500/30 dark:ring-emerald-400/20 backdrop-blur-[12px] [-webkit-backdrop-filter:blur(12px)]",
    error:
      "bg-red-500/20 text-neutral-900 dark:bg-red-400/10 dark:text-neutral-50 ring-1 ring-red-500/30 dark:ring-red-400/20 backdrop-blur-[12px] [-webkit-backdrop-filter:blur(12px)]",
    warning:
      "bg-amber-500/20 text-neutral-900 dark:bg-amber-400/10 dark:text-neutral-50 ring-1 ring-amber-500/30 dark:ring-amber-400/20 backdrop-blur-[12px] [-webkit-backdrop-filter:blur(12px)]",
  ...
  li.className =
    "pointer-events-auto rounded-xl shadow-xl ring-1 ring-black/5 dark:ring-white/10 transition-all duration-300 ease-out transform translate-y-4 opacity-0 data-[open=true]:translate-y-0 data-[open=true]:opacity-100";
  ...
  closeBtn.className =
    "ml-auto shrink-0 rounded-md p-1 hover:opacity-80 focus:outline-none focus:ring-2 focus:ring-white/60";
...
function initializeRailToggle() {
  ...
  const toggleIcon = toggleBtn
    ? toggleBtn.querySelector(".material-symbols-rounded")
    : null;
...
function initializeEverything() {
  console.log(
    "*** UNIFIED NAVIGATION SCRIPT: Starting proper initialization ***"
  );
